...
const COUPONS: Coupon[] = [
  {
    id: "c1",
    title: "20% Off at Nike",
    code: "NIKE20",
    type: "brand",
    details: "Valid on footwear & apparel",
  ...
  },
  {
    ...
    code: "ELEC15",
    type: "category",
    details: "Mobiles, headphones, speakers",
  ...
  {
    id: "c3",
    title: "Flat ₹500 Off at Apple",
    code: "APPLE500",
    type: "brand",
    details: "On selected iPhones & iPads",
...
function CouponCard({ c }: { c: Coupon }) {
  ...
  return (
    <div
      className={`rounded-xl p-[1px] bg-gradient-to-r ${grad} bg-opacity-40`}
    >
      <Card className="relative overflow-hidden group rounded-[11px]">
        <div className={`absolute left-0 top-0 bottom-0 w-1.5 ${leftStrip}`} />
        <div
          className="absolute -top-6 -right-8 w-40 h-40 bg-gradient-to-br from-purple-200/40 to-blue-200/40 rounded-full blur-2xl"
          aria-hidden
        />
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {c.details} • Valid till {c.validTill}
                {c.minPurchase ? ` • Min purchase ${c.minPurchase}` : ""}
              </p>
            </div>
            <Button
              onClick={copy}
              className="rounded-full bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 hover:from-blue-700 hover:via-purple-700 hover:to-pink-700 text-white transition-transform group-hover:scale-[1.02]"
            >
              Copy Code
            </Button>
          </div>
...
export default function Coupons() {
  ...
  const filtered = useMemo(
    () => (tab === "all" ? COUPONS : COUPONS.filter((c) => c.type === tab)),
    [tab],
  );
  return (
      ...
      <section className="bg-gradient-to-r from-amber-100 via-yellow-50 to-orange-100 border-b">
        <div className="container mx-auto px-4 py-10 md:py-14 text-center">
          <h1 className="text-3xl md:text-4xl font-extrabold mb-2">
            Grab Your Best Deals Today!
          </h1>
          <p className="text-gray-700">
            Discover exclusive codes from top brands and categories.
          </p>
        </div>
      </section>
            ...
            <div className="flex items-start gap-3 p-4 rounded-lg bg-white shadow-sm">
              ...
              <div>
                ...
                <div className="text-xs text-muted-foreground">
                  Choose the best offer for your purchase.
                </div>
              </div>
            </div>
            <div className="flex items-start gap-3 p-4 rounded-lg bg-white shadow-sm">
              ...
              <div>
                ...
                <div className="text-xs text-muted-foreground">
                  Tap Copy and it’s saved to your clipboard.
                </div>
              </div>
            </div>
            <div className="flex items-start gap-3 p-4 rounded-lg bg-white shadow-sm">
              ...
              <div>
                ...
                <div className="text-xs text-muted-foreground">
                  Paste the code at checkout to get the discount.
                </div>
              </div>
            </div>
          </div>
