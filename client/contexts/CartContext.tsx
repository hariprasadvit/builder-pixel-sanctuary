...
export function CartProvider({ children }: { children: ReactNode }) {
      ...
      return parsed
        .filter(
          (it: any) =>
            ...
            validVendors.has(it.vendor || "nearbuy"),
        )
        .map((it: any) => ({
          id: it.id,
          name: it.name,
          ...
          shippingWeight:
            typeof it.shippingWeight === "number"
              ? it.shippingWeight
              : undefined,
  ...
  const clearVendorCart = (vendor: MarketplaceType) => {
    ...
    setAppliedCoupons((prev) =>
      prev.filter(
        (coupon) => !coupon.code.toLowerCase().includes(vendor.toLowerCase()),
      ),
    );
  };
  ...
  const getTotals = (vendor?: MarketplaceType): CartTotals => {
    ...
    const deliveryFee = vendor
      ...
      : hasMultipleVendors()
        ? Array.from(
            new Set(items.map((item) => item.vendor || "nearbuy")),
          ).reduce((sum, v) => {
            const vendorSubtotal = getItemsByVendor(
              v as MarketplaceType,
            ).reduce((s, item) => s + item.price * item.quantity, 0);
            return (
              sum + calculateDeliveryFee(v as MarketplaceType, vendorSubtotal)
            );
          }, 0)
        : items.length > 0
          ? calculateDeliveryFee(
              (items[0].vendor || "nearbuy") as MarketplaceType,
              subtotal,
            )
          : 0;
  ...
  const getVendorItemCount = (vendor: MarketplaceType): number => {
    return getItemsByVendor(vendor).reduce(
