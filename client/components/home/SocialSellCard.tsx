...
export default function SocialSellCard(props: SocialSellCardProps) {
  ...
  const socialRow = (
    <div className="flex items-center justify-between text-xs text-gray-600 min-h-[20px] w-full">
      <div className="flex items-center gap-3 flex-1 min-w-0 overflow-hidden">
        ...
        <div className="flex items-center gap-1 whitespace-nowrap">
          <Users className="w-3.5 h-3.5" /> {boughtIn24h} bought in 24h
        </div>
        {typeof liveViewers === "number" && liveViewers > 0 ? (
          <div className="flex items-center gap-1 whitespace-nowrap">
            <Eye className="w-3.5 h-3.5" /> {liveViewers} watching
          </div>
        ) : null}
      </div>
      <button
  ...
  const addToCart = () => {
    ...
    toast({
      title: "Added to cart",
      description: "Added. See similar from creators?",
    });
    ...
    <div className="relative rounded-xl overflow-hidden bg-gray-100 aspect-square">
      {videoSrc ? (
        ...
        <div className="absolute inset-0 grid place-items-center bg-gray-100">
          <span className="rounded-full bg-white p-3 shadow ring-1 ring-black/10">
    ...
    <div className="space-y-2">
      <div className="flex items-center gap-2">
        ...
        <span className="text-sm text-gray-600">
          {rating.toFixed(1)} · {ratingCount.toLocaleString()} reviews
        </span>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        {videoSrc && (
          <Button variant="outline" className="gap-2">
            <Play className="w-4 h-4" /> Watch 15s
          </Button>
        )}
      </div>
    </div>
  ...
  return (
    <div
      className={`group rounded-2xl bg-white shadow-sm overflow-hidden p-4 flex flex-col w-[288px] md:w-[296px] h-[600px]`}
    ...
    >
      ...
      <div className="mt-3 relative">
        {media}
        <div className="absolute top-2 right-2 text-[10px] bg-white/90 text-gray-700 px-2 py-0.5 rounded-full shadow">
          updated {updatedAgo}
        </div>
      </div>

      <div className="mt-3 flex-1 flex flex-col">
        <h3 className="text-base font-semibold line-clamp-2 min-h-[40px]">
          {title}
        </h3>
        ...
        <div className="mt-2 flex items-center gap-2 h-10">
          <Button
            onClick={addToCart}
            className={`${BRAND_GRADIENT} hover:opacity-90 text-white h-9 px-4`}
          >
            {added ? "Added" : "Add to Cart"}
          </Button>
        ...
        <div className="mt-2 text-[11px] text-gray-500 self-start flex items-center gap-1 min-h-[16px]">
