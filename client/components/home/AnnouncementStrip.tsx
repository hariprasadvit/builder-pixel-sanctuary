...
export default function AnnouncementStrip() {
  const items = ["FREE SHIPPING ON ALL ORDERS ABOVE £299", "SALE IS LIVE!"];
  return (
    <>
            ...
            {[0, 1].map((dup) => (
              <div key={dup} className="flex items-center gap-12 pr-12">
                {Array.from({ length: 8 }).map((_, i) => (
                  <span
                    key={`${dup}-${i}`}
                    className="text-white text-[11px] font-semibold tracking-wide uppercase drop-shadow-[0_1px_1px_rgba(0,0,0,0.8)]"
                  >
