...
export default function ReturnWizard({
  ...
  const [slotDay, setSlotDay] = useState<"today" | "tomorrow" | "select">(
    "today",
  );
  ...
  const totalSelected = useMemo(
    () => Object.entries(selected).reduce((s, [id, q]) => s + q, 0),
    [selected],
  );
          ...
          <>
            ...
            {step === 1 && (
              <div className="space-y-3 mt-3">
                    ...
                    <CardContent className="p-3 flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-600">
                          Return by {it.returnBy.toLocaleDateString()}
                        </p>
                        {!it.eligible && (
                      ...
                      <div className="flex items-center gap-2">
                        <Button
                          ...
                          size="icon"
                          disabled={
                            !it.eligible || (selected[it.id] || 0) === 0
                          }
                          onClick={() =>
                            setSelected({
                              ...selected,
                              [it.id]: Math.max(0, (selected[it.id] || 0) - 1),
                            })
                          }
                        >
                          -
                ...
                <p className="text-xs text-gray-500">
                  Non-returnables: perishable, hygiene-sensitive, digital codes.
                </p>
                <Button
                  className="w-full"
            ...
            {step === 3 && (
              <div className="space-y-3 mt-3">
                ...
                <div className="space-y-2">
                  <p className="text-xs text-gray-600">
                    Agent will verify with OTP; have item packed and accessories
                    included. No printer? Agent brings label. Up to 2 pickup
                    attempts before request auto-closes.
                  </p>
                  ...
                  <div className="flex gap-2">
                    {(["today", "tomorrow", "select"] as const).map((d) => (
                      <Button
                        key={d}
                        variant={slotDay === d ? "default" : "outline"}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      ...
                      <Button
                        key={w}
                        variant={slotWindow === w ? "default" : "outline"}
            ...
            {step === 4 && (
              <div className="space-y-3 mt-3">
                ...
                <p className="text-xs text-gray-600">
                  Prepaid 3–7 business days; COD to bank/UPI 5–7 business days.
                </p>
                <div className="flex gap-2">
                  <Button
                    variant="outline"
                ...
                <div className="space-y-2">
                  {eligibleItems
                    ...
                    .map((it) => (
                      ...
                      >
                        <span>
                          {it.name} × {selected[it.id]}
                        </span>
                        <span>
                          {currency}
                          {(it.price * selected[it.id]).toFixed(2)}
                        </span>
                      </div>
                    ))}
                </div>
                <p className="text-xs text-gray-600">
                  Method: Doorstep Pickup — {slotLabel} {slotWindow}. OTP
                  verification. Agent brings label if needed. Up to 2 pickup
                  attempts.
                </p>
          ...
          <>
            <div className="text-center space-y-2">
              ...
              <p className="text-sm text-gray-600">
                Pickup scheduled for {slotLabel} {slotWindow}. Track below.
              </p>
            </div>
            <div className="mt-4">
              <ReturnTimeline
              ...
              <p className="text-xs text-gray-600 mt-3">
                QC outcomes: If pass, refund/exchange proceeds; if failed, we’ll
                send back to you with tracking.
              </p>
            </div>
            <DialogFooter>
